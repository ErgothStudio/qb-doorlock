	...
	while true do
		...
		for k,doorID in ipairs(QB.Doors) do
			...
			if distance < maxDistance then
				...
				local isAuthorized2 = IsAuthorized2(doorID)
				...
				if isAuthorized2 then
					if doorID.locked then
						displayText = "~g~E~w~ - Fechado"
					elseif not doorID.locked then
						displayText = "~g~E~w~ - Aberto"
					end
				elseif not isAuthorized2 then
					if doorID.locked then
						displayText = "Fechado"
					elseif not doorID.locked then
						displayText = "Aberto"
				...
				if QB.Doors[k].pickable then
					if QB.Doors[k].locked then
						if hasItem then
							...
							QBCore.Functions.Notify("Um kit de ferramentas está faltando...", "error")
						end
					else
